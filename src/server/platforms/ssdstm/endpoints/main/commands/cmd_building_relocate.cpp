...
namespace emulator::ssd
{
	json::value cmd_building_relocate::execute(json::value& data, const std::optional<database::users::user>& user)
	{
		...
		if (!map_location_j.is_uint64() ||
			!departure_coordinate_j.is_object() || !departure_edge_type_j.is_uint64() || 
			!destination_coordinate_j.is_object() || !destination_edge_type_j.is_uint64() ||
			!destination_rotation_j.is_uint64())
		{
		...
		const auto departure_edge_type = departure_edge_type_j.as<std::uint32_t>();
		const auto destination_edge_type = destination_edge_type_j.as<std::uint32_t>();
		const auto destination_rotation = destination_rotation_j.as<std::uint8_t>();
		...
		const auto get_coordinate = [](json::value& data, std::uint32_t& col, std::uint32_t& row)
		{
			...
			if (!column_j.is_uint64() || !row_j.is_uint64())
			{
				return false;
			}

			col = column_j.as<std::uint32_t>();
			row = row_j.as<std::uint32_t>();
